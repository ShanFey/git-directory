...
```
С опцией --amend команда commit создаст новый коммит, которым заменит последний. При этом будут учтены изменения как старого коммита, так и нового. 
При выполнении команды создаётся новый коммит, который будет иметь новый хеш. Опция --no-edit сообщает команде commit, что сообщение коммита нужно оставить как было. 
...
Команда позволяет убрать файл из staging обратно в untracked/modified.
...
Команда позволяет «откатить» то, что уже было закоммичено, то есть вернуть состояние репозитория к более раннему. В качестве <commit hash> нужно выбрать последний коммит, ДО того, который надо удалить.
...
Чтобы вернуть всё «как было», можно выполнить команду git restore <file>. Изменения в файле «откатятся» до последней версии, которая была сохранена через git commit или git add.
...
### Игнорирование файлов. Как заполнить .gitignore
